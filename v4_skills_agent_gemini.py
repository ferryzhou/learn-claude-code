#!/usr/bin/env python3
"""
v4_skills_agent_gemini.py - Mini Claude Code: Skills Mechanism (~550 lines) - Gemini Version
...
Usage:
    python v4_skills_agent_gemini.py
"""
...
try:
    ...
    sys.exit("Please install: pip install google-genai python-dotenv")
...
def run_task(description: str, prompt: str, agent_type: str) -> str:
    """Execute a subagent task (from v3)."""
...
Complete the task and return a clear, concise summary."""
    ...
    # Track the last turn to append responses correctly
    
    while True:
        try:
            response = client.models.generate_content(
                model=MODEL,
                contents=sub_messages,
                config=types.GenerateContentConfig(
                    system_instruction=sub_system,
                    tools=[{"function_declarations": sub_tools}],
        ...
        # If no candidates, something went wrong or empty response
        ...
        for fc in function_calls:
            ...
            output = execute_tool(fc.name, args)
        ...
        sub_messages.append({"role": "tool", "parts": tool_outputs})
...
def agent_loop(messages: list) -> list:
    """
    Main agent loop with skills support - Gemini Version.
    """
    while True:
        try:
            response = client.models.generate_content(
        ...
        # In Gemini, we append the raw content object for the assistant turn
        messages.append(content)
        
        # Check for text parts to print
        if content.parts:
            for part in content.parts:
                if part.text:
                    print(part.text)
        ...
        tool_outputs = []
        for fc in function_calls:
            # Convert map keys to strings if needed (Gemini SDK usually handles this)
            args = dict(fc.args) if fc.args else {}
        ...
        messages.append({"role": "tool", "parts": tool_outputs})
...
def main():
    print(f"Mini Claude Code v4 (with Skills) [GEMINI] - {WORKDIR}")
    print(f"Skills: {', '.join(SKILLS.list_skills()) or 'none'}")
    print(f"Agent types: {', '.join(AGENT_TYPES.keys())}")
    print("Type 'exit' to quit.\n")

    # Gemini uses specific structure for history: list of Content or dicts
    history = []
